...
class PTQSLQuantMatMul(MinMaxQuantMatMul):
    ...
    def _get_similarity(self, tensor_raw, tensor_sim, metric=None):
        ...
        elif metric is not None and metric.startswith('fisher'):
            fisher_grad = getattr(self, 'fisher_grad', None)
            if fisher_grad is not None:
                similarity = -(tensor_raw - tensor_sim) ** 2 * fisher_grad.to(tensor_raw.device).abs()
            else:
                similarity = -(tensor_raw - tensor_sim) ** 2
        else:
            similarity = -(tensor_raw - tensor_sim) ** 2
...
class AsymmetricallyBatchingQuantMatMul(PTQSLBatchingQuantMatMul):
    ...
    def hyperparameter_searching(self, fisher_grad=None):
        if fisher_grad is not None:
            self.fisher_grad = fisher_grad
